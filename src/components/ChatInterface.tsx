...
import { Send, Terminal, Cpu, Sparkles, Wand2, Brain, Dot } from "lucide-react";
...
export const ChatInterface = () => {
  ...
  const models = [
    ...
    { id: "claude-3-5", name: "CLAUDE 3.5", icon: <Sparkles size={10} />, color: "text-[#ffaa00]" },
      ...
      try {
        ...
        setMessages((prev) => [
          ...prev,
          {
            id: assistantId,
            role: "assistant",
            content: `ANALYZING PROMPT... GENERATING MODEL CONFIGURATION FOR: "${currentInput}". PLEASE WAIT...`,
            isTyping: true,
          },
        ]);
      } finally {
  ...
  return (
    <div className="w-full h-full flex flex-col glass-morphism rounded-2xl overflow-hidden border border-[#ff007f44] pink-glow-border">
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto px-4 py-6 space-y-4 custom-scrollbar bg-[#060606]"
      >
        <AnimatePresence>
          {messages.map((msg) => {
            if (msg.isThinking) {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`max-w-[78%] md:max-w-[70%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                    isUser
      ...
      <div className="px-4 py-2 flex gap-2 overflow-x-auto shrink-0 border-t border-[#ffffff06]" style={{ scrollbarWidth: "none" }}>
        {prompts.map((p) => (
          <button
            key={p}
            onClick={() => handleAuthRequiredAction(() => setInput(p))}
            disabled={isLoading}
        ...
        <div className="relative flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between px-1">
            <div className="flex gap-2 relative group">
              {/* Dropdown-style trigger (visual only for now, acts as horizontal list on hover if needed, or just stay as buttons) */}
              {models.map((m) => (
                <button
                  key={m.id}
